...
export default function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className={`group bg-white rounded-[20px] border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden flex flex-col active:scale-[0.98] h-full ${isOutOfStock ? 'opacity-90' : ''}`}>
      <Link href={`/product/${product.id}`} className="relative aspect-[4/3] w-full overflow-hidden bg-gray-50/20">
        <Image
          src={product.imageUrl}
          alt={product.name}
          fill
          className={`object-contain p-1.5 group-hover:scale-105 transition-transform duration-500 ${isOutOfStock ? 'grayscale opacity-40' : ''}`}
          data-ai-hint="clinical product"
        />
        {savingsPercent > 0 && !isOutOfStock && (
          <div className="absolute top-1.5 left-1.5 z-10">
            <div className="bg-accent text-white text-[7px] font-black uppercase tracking-widest px-2 py-0.5 rounded-lg shadow-lg">
              {savingsPercent}% OFF
            </div>
          </div>
        )}
      ...
      <div className="p-2.5 flex flex-col flex-1 gap-1">
        <div className="space-y-0.5">
          ...
          <p className="text-[7px] font-black uppercase tracking-widest text-primary/70">{product.packSize}</p>
        </div>
          ...
          <div className="flex items-center justify-between gap-1.5">
            {isOutOfStock ? (
              <Button 
                ...
                variant="outline"
                className="rounded-full h-8 w-full p-0 border-orange-200 bg-orange-50 hover:bg-orange-100 text-orange-600 font-black text-[9px] uppercase tracking-widest gap-1 shadow-sm"
              >
              ...
              <div className="flex items-center gap-1 bg-primary rounded-full p-0.5 shadow-lg shadow-primary/20 w-full overflow-hidden">
                <Button 
                  ...
                  className="h-7 w-7 p-0 rounded-full bg-white/20 text-white hover:bg-white/30 border-none shadow-none shrink-0"
                >
              ...
              <Button 
                onClick={handleAdd} 
                className="rounded-full h-8 w-full p-0 shadow-md bg-primary hover:bg-primary/90 text-white border-none font-black text-[9px] uppercase tracking-widest gap-1.5 active:scale-95 transition-all"
              >
