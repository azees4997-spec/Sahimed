    ...
    >
      <div className="relative aspect-square w-full overflow-hidden bg-white">
        ...
        {savingsPercent > 0 && !isOutOfStock && (
          <div className="absolute top-2 left-2 z-10">
            <div className="bg-accent text-white text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md shadow-lg animate-in fade-in zoom-in duration-500">
              {savingsPercent}% OFF
            </div>
          </div>
        )}
      ...
      <div className="p-4 flex flex-col flex-1">
        <div className="space-y-1 mb-3">
          <h3 className="font-black text-gray-900 line-clamp-2 text-sm uppercase tracking-tight group-hover:text-primary transition-colors leading-tight min-h-[2.5rem]">
            {product.name}
          </h3>
        ...
        <div className="mt-auto space-y-4">
          <div className="flex flex-col gap-0.5">
            <div className="flex items-center justify-between gap-1 flex-wrap">
              ...
              {savingsAmount > 0 && (
                <div className="bg-accent/10 px-2 py-0.5 rounded-md border border-accent/10">
                   <p className="text-[8px] font-black text-accent uppercase whitespace-nowrap">SAVE ₹{savingsAmount}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between gap-2">
            {isOutOfStock ? (
              ...
              <Button 
                onClick={handleAdd} 
                className="rounded-full h-12 w-full bg-primary hover:bg-primary/90 text-white font-black text-[11px] uppercase tracking-[0.1em] gap-3 shadow-md"
              >
