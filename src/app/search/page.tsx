        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between mb-6 px-1">
              <div><h2 className="text-xl font-black text-gray-900 uppercase tracking-tighter">{q ? `"${q}"` : c ? `${c}` : 'Full Catalog'}</h2><p className="text-[8px] font-black text-gray-400 uppercase tracking-widest mt-1">{filtered.length} products found</p></div>
            ...
            {medsLoading ? (
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6">{filtered.map(p => (<ProductCard key={p.id} product={p} />))}</div>
            ) : (
