  ...
  return (
    <div className="min-h-screen bg-[#F8F8F8] pb-32 page-transition-wrapper">
      ...
      <main className="max-w-7xl mx-auto px-4 py-2">
        ...
        {/* High-Fidelity Side-by-Side Comparison */}
        <div className="grid grid-cols-2 gap-3 mb-6">
          {/* Branded Product Card */}
          <Card className="rounded-[24px] border-none bg-white overflow-hidden flex flex-col p-4 shadow-sm relative border border-gray-100">
            <p className="text-[6px] font-black text-gray-400 uppercase tracking-widest mb-3">Branded Reference</p>
            <div className="aspect-square w-full max-w-[100px] bg-gray-50 rounded-2xl mx-auto mb-4 p-2 relative shadow-inner">
              <img src={product?.imageUrl} alt={product?.name} className="w-full h-full object-contain" />
            </div>
            <div className="flex-1 flex flex-col gap-1.5">
              <h3 className="text-[10px] font-black text-gray-900 leading-tight uppercase line-clamp-2 min-h-[2.5em]">{product?.name}</h3>
              <p className="text-[7px] font-bold text-gray-400 uppercase tracking-tight">{product?.manufacturer}</p>
              <p className="text-[8px] font-black text-gray-300 uppercase mt-0.5">{product?.packSize}</p>
                ...
                <div className="mt-4">
                  {brandedQty > 0 ? (
                    <div className="flex items-center justify-between bg-primary rounded-xl h-10 px-2 shadow-lg">
                      <button onClick={() => updateQuantity(product.id, -1)} className="p-1.5 text-white hover:opacity-80 transition-opacity"><Minus className="w-3.5 h-3.5" /></button>
                      <span className="text-[11px] font-black text-white">{brandedQty}</span>
                      <button onClick={() => updateQuantity(product.id, 1)} className="p-1.5 text-white hover:opacity-80 transition-opacity"><Plus className="w-3.5 h-3.5" /></button>
                    </div>
                  ) : (
                    <Button onClick={() => addToCart(product, 1)} className="w-full h-10 rounded-xl text-[9px] font-black uppercase tracking-widest bg-primary hover:bg-primary/90 shadow-xl active:scale-95 transition-all">Add To Cart</Button>
                  )}
                </div>
              </div>
            </div>
          ...
          {genericSubstitute ? (
            <Card className="rounded-[24px] border-2 border-green-500 bg-white overflow-hidden flex flex-col relative p-4 shadow-2xl shadow-green-100">
              <div className="absolute top-0 right-0 z-10">
                <div className="bg-green-500 text-white font-black text-[7px] uppercase px-3 py-1 rounded-bl-xl shadow-lg">
              ...
              <div className="flex-1 flex flex-col gap-1.5">
                <h3 className="text-[10px] font-black text-gray-900 leading-tight uppercase line-clamp-2 min-h-[2.5em]">{genericSubstitute.name}</h3>
                <p className="text-[7px] font-bold text-gray-400 uppercase tracking-tight">{genericSubstitute.manufacturer}</p>
                <p className="text-[8px] font-black text-gray-300 uppercase mt-0.5">{genericSubstitute.packSize}</p>
                  ...
                  <div className="mt-4">
                    {genericQty > 0 ? (
                      <div className="flex items-center justify-between bg-green-600 rounded-xl h-10 px-2 shadow-xl">
                        <button onClick={() => updateQuantity(genericSubstitute.id, -1)} className="p-1.5 text-white hover:opacity-80 transition-opacity"><Minus className="w-3.5 h-3.5" /></button>
                        <span className="text-[11px] font-black text-white">{genericQty}</span>
                        <button onClick={() => updateQuantity(genericSubstitute.id, 1)} className="p-1.5 text-white hover:opacity-80 transition-opacity"><Plus className="w-3.5 h-3.5" /></button>
                      </div>
                    ) : (
                      <Button onClick={() => addToCart(genericSubstitute, 1)} className="w-full h-10 rounded-xl text-[9px] font-black uppercase tracking-widest bg-green-600 hover:bg-green-700 shadow-2xl shadow-green-100 active:scale-95 transition-all">Add To Cart</Button>
                    )}
                  </div>
                </div>
              </div>
            ...
            <Card className="rounded-[24px] border border-dashed border-gray-200 bg-gray-50/50 flex flex-col items-center justify-center p-8 text-center">
              ...
              <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest leading-relaxed">No alternative bio-equivalent available in stock</p>
            </Card>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-white p-4 rounded-[24px] border border-gray-100 shadow-sm">
            <h3 className="text-[9px] font-black text-gray-900 uppercase tracking-widest mb-3 flex items-center gap-2">
              <Activity className="w-4 h-4 text-primary" /> Therapeutic Uses
            </h3>
            <ul className="space-y-2">
              {(product?.uses || []).slice(0, 3).map((use: string, i: number) => (
                <li key={i} className="text-[9px] font-bold text-gray-600 leading-tight flex gap-2">
          ...
          <div className="bg-white p-4 rounded-[24px] border border-gray-100 shadow-sm">
            <h3 className="text-[9px] font-black text-gray-900 uppercase tracking-widest mb-3 flex items-center gap-2">
              <AlertCircle className="w-4 h-4 text-orange-500" /> Side Effects
            </h3>
