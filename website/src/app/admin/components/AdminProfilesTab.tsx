...
export function AdminProfilesTab({ db, isVerified, onBack }: { db: any, isVerified: boolean, onBack: () => void }) {
  ...
  const adminsQuery = query(collection(db, 'adminProfiles'), orderBy('activatedAt', 'desc'));
  const { data: adminList, isLoading } = useCollection(adminsQuery);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [newAdmin, setNewAdmin] = useState({ uid: '', role: 'sub-admin', name: '', email: '' });

  const handleCreateAdmin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newAdmin.uid) return;

    try {
      await setDocumentNonBlocking(doc(db, 'adminProfiles', newAdmin.uid), {
        id: newAdmin.uid,
        role: newAdmin.role,
        name: newAdmin.name,
      ...
      toast({ title: "Authority Granted", description: `User role set to ${newAdmin.role}` });
      setIsFormOpen(false);
  ...
  const updateRole = (uid: string, newRole: string) => {
    setDocumentNonBlocking(doc(db, 'adminProfiles', uid), { role: newRole }, { merge: true });
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {isLoading ? (
          ...
          <Card key={admin.id} className="rounded-[44px] p-10 border-none shadow-xl bg-white group hover:shadow-2xl transition-all border border-white relative overflow-hidden">
            ...
            <div className="space-y-6 relative z-10">
              <div className="flex items-center justify-between">
                ...
                <Badge className={cn("rounded-full font-black text-[8px] px-3", 
              ...
              <div className="pt-4 border-t border-slate-50">
                <Select defaultValue={admin.role} onValueChange={(val) => updateRole(admin.id, val)}>
                  <SelectTrigger className="w-full h-12 rounded-2xl bg-slate-50 border-none font-bold text-[10px] uppercase">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="flex items-center justify-between pt-4">
                ...
                <Button variant="ghost" size="icon" onClick={() => { if(confirm("Revoke all access?")) deleteDocumentNonBlocking(doc(db, 'adminProfiles', admin.id)); }} className="text-red-300 hover:text-red-500 hover:bg-red-50 rounded-xl">
