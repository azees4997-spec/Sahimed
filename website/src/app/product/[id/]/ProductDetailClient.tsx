...
import { scaleInVariant, fadeInVariant } from '@/lib/animations';
...
export default function ProductDetailClient({ initialProduct, id }: { initialProduct: any, id: string }) {
  ...
  // FETCH ALTERNATIVES (STRICT MANUAL ID ONLY)
  ...
  const brandedPrice = Number(product?.liveData?.sahimed_price || product?.price || 0);
  const brandedMrp = Number(product?.liveData?.mrp || product?.mrp || brandedPrice + 20);
  ...
  const showComparison = !isGeneric && !!genericAlt && switchSavingsAmt > 0;

  if (!product && productLoading) {
    return (
      <div className="min-h-screen bg-[#F8FAFC]">
        <Navbar />
        <main className="max-w-7xl mx-auto px-4 py-12"><div className="space-y-8"><Skeleton className="h-[300px] rounded-[40px]" /><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><Skeleton className="h-[200px] rounded-[32px]" /><Skeleton className="h-[200px] rounded-[32px]" /></div></div></main>
      </div>
    );
  }
  ...
  return (
    <PageTransition>
      <div className="min-h-screen bg-[#F8FAFC] pb-32">
        <Navbar />
        <main className="max-w-[1000px] mx-auto px-2 sm:px-10 py-1 sm:py-6">
          <div className="flex flex-row items-center justify-center mb-1 sm:mb-4 gap-4 px-2">
             {(product?.prescriptionRequired || product?.rxRequired) && (
               <Badge className="bg-rose-500 text-white border-none rounded-full font-black text-[7px] sm:text-[9px] px-2 py-0.5 tracking-widest uppercase shrink-0">RX REQUIRED</Badge>
             )}
          </div>
          ...
          {showComparison && (
            <motion.div initial={{ scale: 0.98, opacity: 0, y: -5 }} animate={{ scale: 1, opacity: 1, y: 0 }} className="mb-2 sm:mb-6 px-2">
              <div className="bg-gradient-to-r from-primary to-accent text-white py-1.5 sm:py-2.5 px-4 sm:px-8 rounded-[12px] sm:rounded-[20px] shadow-lg flex items-center justify-center gap-2 text-center">
                 ...
                 <h2 className="text-[8px] sm:text-[11px] font-black tracking-widest uppercase line-clamp-1">Switch and save ₹{Number(switchSavingsAmt).toFixed(0)} • Same Medicine</h2>
              </div>
            </motion.div>
          )}
            ...
            {showComparison ? (
              ...
              <div className="flex justify-center">
                <div className="w-full sm:w-[500px]">
                  <ComparisonCard 
                    product={product} 
                    label={isGeneric ? "Sahi Recommended" : "Selected Item"} 
                    getItemQuantity={getItemQuantity} 
                    addToCart={addToCart} 
                    showComparison={false} 
                    brandedMrp={brandedMrp} 
          ...
          <motion.section initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-white rounded-[24px] sm:rounded-[48px] p-4 sm:p-12 shadow-sm border border-slate-100 overflow-hidden">
            <Tabs defaultValue="clinical" className="w-full">
              ...
              <TabsContent value="clinical" className="space-y-10 focus-visible:outline-none">
                 <div className="max-w-5xl mx-auto grid grid-cols-2 md:grid-cols-2 gap-3 sm:gap-8">
                    <div className="bg-lavender p-5 sm:p-10 rounded-[24px] sm:rounded-[40px] border border-white space-y-2 sm:space-y-4 shadow-sm">
                       <div className="flex items-center gap-2 sm:gap-4"><ClipboardList className="w-4 h-4 sm:w-5 sm:h-5 text-primary" /><h3 className="text-[10px] sm:text-lg font-black text-slate-800 tracking-tighter uppercase">Medical Uses</h3></div>
                       <p className="text-[8px] sm:text-[11px] font-black text-slate-500 leading-tight uppercase opacity-70 tracking-tight">{product?.treatment || "Standard medical use."}</p>
                    </div>
                    <div className="bg-sahi-blue p-5 sm:p-10 rounded-[24px] sm:rounded-[40px] border border-white space-y-2 sm:space-y-4 shadow-sm">
                       <div className="flex items-center gap-2 sm:gap-4"><Info className="w-4 h-4 sm:w-5 sm:h-5 text-primary" /><h3 className="text-[10px] sm:text-lg font-black text-slate-800 tracking-tighter uppercase">Product Info</h3></div>
                       <p className="text-[8px] sm:text-[11px] font-black text-slate-500 leading-tight uppercase opacity-70 tracking-tight">{product?.description || "Medicine details."}</p>
                    </div>
                  </div>
              </TabsContent>
              ...
              <TabsContent value="safety" className="grid grid-cols-2 md:grid-cols-2 gap-3 sm:gap-8 focus-visible:outline-none">
                 <div className="bg-sahi-pink border border-white p-5 sm:p-10 rounded-[24px] sm:rounded-[40px] flex flex-col sm:flex-row gap-4 sm:gap-6 shadow-sm">
                   <div className="w-10 h-10 sm:w-16 sm:h-16 bg-white rounded-[24px] flex items-center justify-center shrink-0 shadow-lg"><AlertTriangle className="w-4 h-4 sm:w-6 sm:h-6 text-rose-500" /></div>
                   <div><h4 className="text-[8px] sm:text-xs font-black text-rose-600 mb-2 uppercase tracking-widest">Safety Advice</h4><p className="text-[8px] sm:text-[10px] font-black text-rose-900/60 leading-tight uppercase tracking-tight">{product?.safetyAdvice || "Follow medical guidance."}</p></div>
                 </div>
                 <div className="bg-sahi-blue border border-white p-5 sm:p-10 rounded-[24px] sm:rounded-[40px] flex flex-col sm:flex-row gap-4 sm:gap-6 shadow-sm">
                   <div className="w-10 h-10 sm:w-16 sm:h-16 bg-white rounded-[24px] flex items-center justify-center shrink-0 shadow-lg"><Stethoscope className="w-4 h-4 sm:w-6 sm:h-6 text-primary" /></div>
                   <div><h4 className="text-[8px] sm:text-xs font-black text-primary mb-2 uppercase tracking-widest">How to Use</h4><p className="text-[8px] sm:text-[10px] font-black text-slate-500 leading-tight uppercase tracking-tight">{product?.howToUse || "Take as directed by your doctor."}</p></div>
                 </div>
              </TabsContent>
              ...
              <TabsContent value="interactions" className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6 focus-visible:outline-none">
                {[
                  ...
                  <div key={i} className={cn("p-4 sm:p-8 rounded-[24px] sm:rounded-[32px] flex flex-col gap-3 sm:gap-5 border border-white shadow-sm", item.color)}>
                    <div className="w-8 h-8 sm:w-12 sm:h-12 bg-white rounded-[12px] flex items-center justify-center text-primary shrink-0 shadow-sm border border-slate-50"><item.icon className="w-4 h-4 sm:w-5 sm:h-5" /></div>
