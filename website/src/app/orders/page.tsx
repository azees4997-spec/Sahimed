      ...
      <main className="max-w-4xl mx-auto px-4 py-6 md:py-16">
        <div className="flex items-center gap-4 mb-10">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[10px] font-black text-gray-400 tracking-widest leading-none">Your clinical records</p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          {isLoading ? (
            <div className="text-center py-24">
              ...
              <p className="text-gray-400 font-bold text-[9px] tracking-[0.2em]">Syncing health records...</p>
            </div>
                ...
                <CardHeader className="p-8 border-b bg-gray-50/30">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[9px] font-black text-gray-400 tracking-widest mb-0.5">Order ref</p>
                        <h3 className="font-black text-sm text-gray-900 tracking-tight">#{order.id.substring(0, 12).toUpperCase()}</h3>
                      </div>
                    </div>
                    <div className="flex items-center gap-6">
                      <div className="text-right">
                        <p className="text-[9px] font-black text-gray-400 tracking-widest mb-0.5">Final value</p>
                        <p className="font-black text-lg text-primary tracking-tighter">₹{formatCurrency(order.totalAmount)}</p>
                      </div>
                      <Badge className={cn(
                        "rounded-full px-5 py-1.5 text-[9px] font-black tracking-widest border-none",
                ...
                <CardContent className="px-8 py-6">
                  <div className="flex flex-wrap items-center gap-8">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-bold text-gray-500 tracking-widest">{order.paymentType || 'COD'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <MapPin className="w-3.5 h-3.5 text-gray-400" />
                      <span className="text-[10px] font-bold text-gray-500 tracking-widest truncate max-w-[200px]">{order.shippingDetails?.street || 'Verified address'}</span>
                    </div>
                    {order.prescriptionUrl && (
                      <div className="flex items-center gap-2 text-primary">
                        <ImageIcon className="w-3.5 h-3.5" />
                        <span className="text-[10px] font-black tracking-widest">Prescription attached</span>
                      </div>
                    )}
                  </div>
                </CardContent>
            ...
            <div className="text-center py-20 bg-white rounded-[40px] border-none shadow-sm">
               ...
               <h2 className="text-2xl font-black mb-2 tracking-tight text-gray-900">No health journeys yet</h2>
               <p className="text-gray-400 font-bold mb-10 text-[10px] tracking-widest">Your clinical orders will appear here.</p>
               <Link href="/">
                 <Button className="rounded-full px-12 h-16 font-black text-sm shadow-2xl shadow-primary/20 tracking-widest active:scale-95 text-white bg-primary">Start shopping</Button>
               </Link>
            </div>
          )}
        </div>
          ...
          <div className="p-8 md:p-10 space-y-10 max-h-[75vh] overflow-y-auto scrollbar-hide">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-4">
                <h4 className="text-[10px] font-black tracking-widest text-gray-400 flex items-center gap-2">
                  <User className="w-3 h-3" /> Recipient
                </h4>
                <div className="bg-gray-50 p-5 rounded-[24px] border border-gray-100">
                  <p className="font-black text-sm text-gray-900">{selectedOrder?.patientName}</p>
                    ...
                    <div className="relative aspect-[16/9] w-full bg-gray-50 rounded-[32px] overflow-hidden border-2 border-dashed border-gray-200 cursor-zoom-in group">
                      ...
                      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/5 transition-colors flex items-center justify-center">
                        <div className="bg-white/90 backdrop-blur-sm px-4 py-2 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-2">
                          ...
                          <span className="text-[8px] font-black tracking-widest">Click to expand</span>
                        </div>
                      </div>
                    </div>
                  </DialogTrigger>
              ...
              <div className="border rounded-[32px] overflow-hidden">
                <table className="w-full text-left">
                  <thead className="bg-gray-50 border-b">
                    <tr>
                      <th className="px-6 py-4 text-[9px] font-black text-gray-400">Medicine</th>
                      <th className="px-6 py-4 text-[9px] font-black text-gray-400 text-center">Qty</th>
                      <th className="px-6 py-4 text-[9px] font-black text-gray-400 text-right">Price</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-50">
                    {selectedOrder?.items?.map((item: any, i: number) => (
                      <tr key={i}>
                        <td className="px-6 py-5 font-black text-xs text-gray-900">{item.name}</td>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 p-8 rounded-[40px] border border-gray-100 space-y-4">
                ...
                <div className="flex justify-between text-[11px] font-black text-accent">
                ...
                <div className="pt-6 border-t border-dashed flex justify-between items-baseline">
                  <span className="text-sm font-black text-gray-900 tracking-tighter">Net payable</span>
            ...
            <Button 
              ...
              className="w-full h-16 rounded-full font-black tracking-widest text-[11px] bg-gray-900 text-white hover:bg-gray-800 transition-all shadow-xl"
            >
              Close record
            </Button>
          </div>
